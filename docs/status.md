...
- 前端已新增轻量 Node 渲染测试，覆盖当前点 prompt 面板、Request Inspector、生产总览指标、正文与结构页入口、阶段入口按钮、分段审片当前点按钮属性、场景工作台按钮属性、状态矩阵、请求调试文档过滤、时间线数据归一化、任务状态判定、连续性展示、文档与总片预览、通用提示 helper 和时间线关键生成按钮
