...
- 生成 scene 级场景母图，用于锁定地点、光线、透视、背景锚点和固定道具；跨 scene 同空间推进时参考上一场母图生成新的派生母图。
- 小说转视频支持九宫格分镜模式：每个 segment 先生成 3x3 连续分镜图，九格展开为起始、推进、结果关键帧，再把九宫格作为 Seedance 的主要视频参考图，需要承接时额外提交上一段尾帧。
- 连续表演模式可直接提交当前场景母图、实际出镜角色图和可用的上一段尾帧生成 Seedance 视频。
...
- 小说转视频：输入项目名、故事创意和生成参数，选择连续表演或九宫格分镜模式，创建视频项目；角色图、场景母图和九宫格分镜图每次生成前可选择 GPT Image 2 或 Seedream 4.5。
...
- 分段审片台：查看 segment 九宫格、视频、资源图、连续性问题、视频 prompt 和重跑操作。
- 合并交付页：查看完整成片和片段资产。
...
- 九宫格分镜模式下，九宫格生图只使用场景母图和角色图；如果规划判断当前视频需要承接上一段，Seedance 视频阶段会提交九宫格图和尾帧图。
- 连续表演模式下，上一段视频尾帧只在连续承接时作为当前片段的开场时间锚点。
- Seedance prompt 会根据真实提交顺序写明 `图片1 / 图片2 / 图片3` 的用途。
- 同一 scene 内多个 segment 共用当前 scene 母图；跨 scene 同空间推进时生成新的派生母图；新地点、空间关系变化明确或连续性不确定时生成新场景母图。
...
- `storyboard_grid_manifest.json`：九宫格分镜任务、参考图绑定、分场景描述和结果。
...
- [九宫格分镜模式开发文档](docs/novel_to_video_grid_storyboard_mode.md)
- [当前状态](docs/status.md)
