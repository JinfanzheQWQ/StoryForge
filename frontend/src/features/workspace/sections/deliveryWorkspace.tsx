...
export function DeliveryWorkspace({ artifacts }: { artifacts?: ArtifactBundle }) {
  ...
  return (
    <section className="library-workspace delivery-workspace" aria-label="合并交付">
      ...
      <section className={fullStory?.url ? "delivery-final-film ready" : "delivery-final-film"} aria-label="完整成片">
        {fullStory?.url ? (
          <>
            <div className="delivery-final-player">
              <video controls src={fullStory.url} aria-label="完整成片">
                <track kind="captions" />
              </video>
            </div>
            <div className="delivery-final-copy">
              ...
              <a className="primary-link" href={fullStory.url} target="_blank" rel="noreferrer">
                打开完整视频
              </a>
            </div>
          </>
