...
- 项目详情只暴露当前五个工作台 tab；文档、图片、视频的独立资产面板由请求调试、场景工作台和分段审片台承接
...
- 前端详情页已开始拆分：Prompt Editor / Request Inspector / Prompt Diff 已从 `render/detail_assets.js` 抽到 `render/prompt_tools.js`，生产总览已抽到 `render/overview.js`，正文与结构页已抽到 `render/story_structure.js`，阶段入口已抽到 `render/run_stage_actions.js`，分段审片台主体已抽到 `render/segment_review.js`，场景工作台主体已抽到 `render/scene_workbench.js`，请求与调试页已抽到 `render/request_debug.js`，时间线界面已抽到 `render/timeline.js`，时间线数据归一化已抽到 `render/timeline_data.js`，任务状态与按钮文案已抽到 `render/task_state.js`，连续性与修复提示展示已抽到 `render/continuity_ui.js`，文档与总片预览展示已抽到 `render/document_assets.js`，详情页通用展示 helper 已抽到 `render/detail_common.js`
- 前端已新增轻量 Node 渲染测试，覆盖当前点 prompt 面板、Request Inspector、生产总览指标、正文与结构页入口、阶段入口按钮、分段审片当前点按钮属性、场景工作台按钮属性、状态矩阵、请求调试文档过滤、时间线数据归一化、任务状态判定、连续性展示、文档与总片预览、通用提示 helper、详情页主入口 helper 注入、前端模块结构契约和时间线关键生成按钮
