...
class ConfigTestCase(unittest.TestCase):
    def test_load_example_config(self) -> None:
        ...
        self.assertFalse(config.seedream.watermark)
        self.assertEqual(config.gpt_image.provider, "kie")
        self.assertEqual(config.gpt_image.model, "gpt-image-2")
        self.assertEqual(config.gpt_image.kie_api_key_env, "KIE_API_KEY")
        self.assertEqual(config.gpt_image.openai_api_key_env, "OPENAI_API_KEY")
        self.assertEqual(config.seedance.model, "doubao-seedance-2-0-260128")
        self.assertFalse(config.seedance.auto_submit)
        self.assertFalse(config.seedance.watermark)
