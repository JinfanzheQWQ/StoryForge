...
在场景工作台或分段审片台按 scene / segment 生成图片：

- scene master frame 是无角色场景母图。
- start frame 是片段首帧。
- mid frame 是需要时生成的中段锚点帧。
- end frame 是片段尾帧。

单个图片可以单独修改 prompt 并重做。页面会展示该图片真实提交给 Seedream 的 prompt、参考图和 payload。

### 7. 生成视频

当某个 segment 所需关键帧就绪后，可以生成该段视频。

Seedance 视频提交使用片段关键帧作为时间锚点：

- `图片1`：首帧
- `图片2`：中段帧，如该段需要中段帧
- `图片3`：尾帧

如果该段没有中段帧，提交顺序会按实际可用关键帧绑定。页面的 Request Inspector 会显示真实图片顺序和完整请求参数。
...
它可以帮助判断问题发生在规划、prompt 组装、参考图顺序、动作容量、时长预算、关键帧质量还是 provider 执行阶段。
...
- 视频失败时，先检查关键帧是否就绪、图片顺序、视频 prompt 和 Seedance 返回。
