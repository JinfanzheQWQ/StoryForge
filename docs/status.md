...
- scene skeleton 现在还会保留紧凑版 `covered_event_summaries`，供后续 chunk planner / repair / 边界校验复用
- 非首个 scene 现在还带 `scene_transition_contract`，用于描述它如何从上一场进入当前场
- `scene skeleton -> 最终 scene_plan` 的 stage2 回写链路现在会保留 `scene_transition_contract` 与 `covered_event_summaries`，不再在重建 / 归一化时丢掉 scene 级边界信息
...
- `Chapter Event Planner` 的尾部覆盖校验已经收紧到短章：不再只检查长章，较短章节如果最后一个 must-cover event 结束得过早，也会直接失败重试
...
- 默认 `mid_frame_prompt` 现在优先取 `timed_beats` 的中间拍，不再回灌整段片段总述，避免中段锚点帧被未来状态污染
- `mid/end` 帧的 frame-level prompt 上下文已进一步收紧：不再直接注入整段 `shot_state.action_progression` 或 `continuity_link.allowed_changes`，而是只保留当前帧真正需要的 `blocking / framing / end_state_lock` 与通用承接语句
- `scene_master_frame` prompt 过滤已补强：`两人 / 双人 / 剪影 / 并肩 / 相对 / 接吻` 这类弱人物信号也会被判成人物内容并从母图 prompt 中移除
- `scene_master_frame` 现在会从 `scene_bible.spatial_layout / character_blocking / scene_anchor / 关键帧 prompt` 中提取环境版空间合同，尽量保留 `长椅 / 画架 / 步道 / 十米外 / 右后方` 这类真实空间关系，同时剔除角色名和人物动作词
...
- segment 后处理与 Seedance 归一化现在都会保留 scene 级元数据，不再因为按扁平 `segments` 重建 plan 而丢失 `covered_event_ids` 等 scene 级字段
...
- scene 首个 chunk / 首个 segment 现在会显式消费 `scene_transition_contract`
...
- `continuity_report` 的 `V1` 规则审校现在也会直接检查 scene 边界：
...
- chapter event planner 现在也会限制单个关键事件的粒度：一个 event 最多只保留 1-2 个紧密绑定推进点；如果一个 event 已经把“等待 -> 会面 -> 开口”或“试探 -> 告白 -> 回应”写成同一个 summary，会在 chapter 结构化阶段直接失败重试
- 多 event 章节的首尾 event 现在允许最多 3 个紧密绑定推进点，用来容纳常见的开场建立与章节收束；中间 event 与单 event 章节仍维持 2 点上限
- chapter event prompt / retry 现在也会显式排除“只用于解释上下文的背景介绍、关系说明、回忆补叙”；中间 event 不能再把问句、回答和动作结果三连塞进同一个 event
- chapter event 的粗粒度估算现在以 `summary` 为主判，不再把同一 event 的 `source_evidence` 碎片重复算成额外推进点；`source_evidence` 也被收口为当前 event 对应的 1-2 个相邻正文片段
- chapter event 如果第一次仍产出“过粗 event”，当前链路会自动进入 `video-chapter-event-repair` 定向修复，不再只依赖全量重试
...
- chapter event 粗粒度修复现在是迭代式的：如果同一章里连续存在多个粗 event，系统会在每轮 repair / split repair 合并后继续校验整章，并对新的粗 event 继续下一轮修复，而不是只修第一处
- `video-chapter-event-split-repair` 的本地校验现在只强校验当前 replacement events 本身，不会因为整章里另一个尚未处理的粗 event 而阻塞当前 targeted split
- chapter scene planner prompt 现在也会显式提醒：不要把过多相邻关键事件一口吞进同一个 scene；如果事件块已经形成完整多阶段链路，应优先拆更多 scene，而不是把压力全留给 chunk planner
- scene chunk prompt 现在把 `expected_segment_count` 明确为保守上限而非目标值，并强调“一个 chunk = 一个连续事件目标”，减少把同一动作链拆成多个近义 chunk
- scene chunk prompt 现在还会显式写出当前 scene 绑定的 `covered_event_ids`，并要求最后一个 chunk 真正落到本 scene 的最后结果，不能只停在“即将发生”
- scene chunk prompt 与结构化校验现在都会同时读取当前 scene 绑定的 `covered_event_summaries`；如果 chunk 偷偷写进后续 scene 才该发生的关键推进，会按 scene 边界越界直接失败重试
- scene chunk 结构化校验现在也会前置检查动作容量：会按 `must_cover / transition_goal` 估算推进点数量；如果一个 chunk 明显已经包含多轮动作结果，但 `expected_segment_count` 仍过小，会直接失败重试并要求拆更多 segment 或更早拆 chunk
- 如果 scene chunk 的常规重试已经耗尽，但最后失败仍是“动作容量过载”，系统现在会自动追加一次 `video-scene-chunk-repair` 定向修复，只要求模型修当前失败 chunk 的段数预算，而不是整 scene 盲重跑
- segment contract prompt 现在明确 `expected_segment_count` 不必凑满，`title / summary` 禁止写 `第1段 / 继续 / 延续` 这类工程化标签或弱变化描述
- segment contract prompt 现在要求：如果当前 chunk 的 `must_cover / transition_goal` 已经是开口、回应、靠近、牵手、拥抱、亲吻或离开决定，最后一个 segment 必须真正落到这个结果，不要只写“准备做”
- `scene chunk -> segment contract` 现在仍会检查最后一个 segment 是否真正落到当前 chunk 的 `transition_goal`；这类“收束不够落地”的问题会优先走 repair，repair 仍失败时降级成 planner warning，不再默认卡死主链路
- 如果 scene segment planner 的常规重试已经耗尽，但最后失败仍是“某个 segment 的 `timed_beats` 尾部没有覆盖完整时长”，系统现在会自动追加一次 `video-scene-segment-timeline-repair`，只定向补该段尾部 beat，而不是整 chunk 直接失败
- 如果 scene segment planner 的常规重试已经耗尽，但最后失败仍是“某个 segment 的动作容量过载”，系统现在会自动追加一次 `video-scene-segment-action-repair`，只定向拆该段的动作链，而不是整 chunk 直接失败
- `video-scene-segment-action-repair` 现在也会迭代消化后续失败：如果第一轮 repair 后仍残留过载子段，或 repair 已经拆出更多段但被旧的 chunk 段数上限卡住，系统会继续拿最新失败 batch 再跑下一轮，而不是直接停在 repair 自身的 3 次失败
- 如果 scene segment planner 的常规重试已经耗尽，但最后失败仍是“多人同帧却仍要求单人特写”，系统现在会自动追加一次 `video-scene-segment-focus-repair`，只定向修该段的共享镜头字段，而不是整 chunk 直接失败
...
- segment contract prompt 现在明确 `opening_match` 必须写成可拍到的开场状态，并给出 `start / continue` 两类简短示例；这类开场承接偏弱问题现在也会优先 repair，必要时只记 warning
...
- 无中段片段现在也会做 `start -> end` 关键帧语义距离检查；如果首尾只是“停在原地 / 保持等待 / 姿态几乎不变”的近义改写，会优先触发 repair，仍失败时降级成 warning
...
- segment 合同阶段现在也会直接校验动作容量：5-6 秒片段最多允许 1-2 个可见推进点，8-12 秒最多 2-3 个；如果 `timed_beats` 把多轮动作、对白轮次或关系变化硬塞进同一段，会直接触发 chunk 内拆段重试
...
- 非首个 scene 的首段首帧现在会额外带上一场最后一段的尾帧作为 temporal anchor，用来帮助跨场过桥；但不会跨 scene 直接复用上一场尾帧成图
...
- 视频基础 prompt 现在直接输出 `参考图`：用 `图片1 / 图片2 / 图片3` 描述起步、中段、收束画面，再按开场 / 中段 / 收束分阶段写 `画面推进`，并优先消费 `timed_beats` 里的秒数与动作描述，明确镜头如何从前一张自然推进到后一张
- 如果当前段存在 `dialogue_lines / narration`，视频 prompt 现在会把真实发生的旁白 / 对白直接挂到对应的 `画面推进` 阶段里，而不只是在后面单独列一份台词清单
- 上游 `scene segment planner` 与 `segment continuity repair` prompt 现在也会显式要求：只要当前段存在 `dialogue_lines / narration`，`timed_beats` 就必须直接写出哪一秒谁说了哪句，不能再只写“他开口 / 她回应”
- Seedance 提交层只补一小段 `提交素材绑定`，按本次请求的真实图片顺序说明哪一张是首帧 / 中段 / 尾帧
- 如果某段是非首个 scene 的首段，Seedance prompt 现在还会追加一小段跨 scene 承接指令：先长成 `next_scene_entry_match`，再执行 `bridge_action / visual_bridge`，并按 `audio_bridge` 保持开场音频尾韵
...
- 当前视频基础 prompt 也已去掉 `片段标题 / 场景与基线 / 镜头与动作` 这类大段重复栏目，收敛成“参考图 + 画面推进 + 音频字幕约束”的短版结构
- 视频片段支持单段生成
