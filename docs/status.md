...
- Seedream 单帧生图 prompt 使用短版图片绑定风格：`图片1` 固定是 `scene_master_frame`，`图片2 / 图片3` 固定是当前帧角色参考，时间承接帧只作为追加连续性参考放在后面
...
- 视频阶段不提交所有额外辅助参考图提交；Seedance 只吃首帧 / 中段 / 尾帧三张时间锚点图，避免母场景图或角色图继续干扰视频构图
- 中段锚点图是 Seedance 默认主路径的一部分，不会只是接口降级时才会带上的兜底参考图
- 视频基础 prompt 直接输出 `参考图`：用 `图片1 / 图片2 / 图片3` 描述起步、中段、收束画面，再按开场 / 中段 / 收束分阶段写 `画面推进`，并优先消费 `timed_beats` 里的秒数与动作描述，明确镜头如何从前一张自然推进到后一张
